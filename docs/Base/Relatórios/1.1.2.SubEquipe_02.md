...
### Rastreabilidade e Versionamento dos Artefatos

O SIG e o Rich Picture passaram por múltiplas iterações de revisão colaborativa entre os dias 25 e 27 de agosto, evidenciadas pelo histórico de atividade e pelo histórico de versões do Figma, com edições registradas pelos três integrantes da subequipe (Ana Luiza Borba, Enzo e Pablo Cunha).

<center>

<p style={{ textAlign: 'center', fontSize: '18px' }}><b>Figura 5</b> - Histórico de Versões do Figma &#8212; Atividade dos Integrantes (25–27/08)</p>

![Histórico de Atividade](../../assets/SubEquipe_02/h1.png)
...
<p style={{ textAlign: 'center', fontSize: '18px' }}><b>Figura 6</b> - Histórico de Versões do Figma &#8212; Registro Automático de Versionamento</p>

![Histórico de Versões](../../assets/SubEquipe_02/h2.png)

<p style={{ textAlign: 'center', fontSize: '17px' }}>Autores: Integrantes da SubEquipe 02</p>

</center>

**Principais correções aplicadas ao longo dessas iterações:**

- Correção da origem da seta de correlação negativa (trade-off) no SIG, que inicialmente saía do softgoal geral "Engajamento" e foi redirecionada para a decisão específica "Pomodoro com XP", que é a causa real do efeito colateral;
- Ajuste dos pesos de contribuição (+/++) para refletir a real importância de cada decisão de design, com destaque para o Pomodoro com XP, elevado para `++` por ser a funcionalidade central da proposta;
- Padronização de idioma, com tradução de todos os rótulos de softgoal de "for X" (inglês) para preposições em português ("da/do X");
- Adição do softgoal `Consistência da Simulação Offline`, identificado como lacuna ao comparar o SIG com a descrição do comportamento do App no Rich Picture.
...
Para representar o comportamento do dispositivo original, obtido por meio da 
Engenharia Reversa, a modelagem em **BPMN 2.0** foi estruturada em duas raias 
(*Lanes*) dentro de um único *Pool*, representando o Jogador e o próprio 
Tamagotchi como participantes do processo:

<center>

<p style={{ textAlign: 'center', fontSize: '18px' }}><b>Figura 4</b> - Modelagem 
BPMN do Ciclo de Interação do Tamagotchi Original (Engenharia Reversa)</p>

![Diagrama BPMN do Tamagotchi Original](../../assets/SubEquipe_02/BPMN2.jpeg)

<p style={{ textAlign: 'center', fontSize: '17px' }}>Autores: Integrantes da 
SubEquipe 02</p>
...
1. **Raia: Jogador**
   * Ao ouvir o bipe de alerta (buzzer piezoelétrico, T1), o usuário verifica 
   o ícone piscando indicando a necessidade ativa e pressiona um dos três 
   botões físicos (SW1 a SW3) para navegar no menu de ações.

2. **Raia: Tamagotchi**
   * O dispositivo identifica a necessidade com base no ícone ativo e 
   direciona o fluxo por um gateway exclusivo entre **Fome**, **Sujeira** ou 
   **Doença**, atendendo a necessidade correspondente (refeição, limpeza ou 
   remédio).
   * Conforme descrito na Decomposição Crítica (Figura 3), os status decaem 
   automaticamente segundo as janelas T2 (cuidado) e T3 (sono), sem qualquer 
   mecanismo de pausa — fator crítico que gerava bipes inoportunos e estresse 
   no usuário.
   * O acúmulo de poucos "care mistakes" ao longo do tempo determina a 
   evolução do pet entre as 4 formas adultas (D1a a D1d), de maneira opaca 
   para o jogador.
   * A negligência prolongada pode deixar o pet doente, triste ou levá-lo à 
   "morte" — comportamento que, na Figura 4.1, é substituído pelas 
   notificações assíncronas do produto proposto.

### Comparativo: BPMN do Produto Proposto

Como forma de evidenciar de que maneira cada decisão de redesign responde 
diretamente aos pontos críticos identificados na Decomposição Crítica (Figura 
3) e no ciclo original do Tamagotchi (Figura 4), apresenta-se a seguir o fluxo 
BPMN do produto proposto pela equipe, contemplando o módulo de cuidados, o 
Parceiro de Estudo (Pomodoro) e a Loja de XP com Controle Parental.

<center>

<p style={{ textAlign: 'center', fontSize: '18px' }}><b>Figura 4.1</b> - 
Modelagem BPMN do Fluxo Integrado de Cuidados, Foco Pomodoro e Compras 
(Produto Proposto)</p>

![Diagrama BPMN do Produto Proposto](../../assets/SubEquipe_02/BPMN1.png)
...
1. **Pool: Jovem (Usuário):**
   * *Ações de Cuidado:* O processo se inicia com a necessidade de cuidar do 
   pet, onde o usuário seleciona e executa ações de alimentação, medicação, 
   higiene e descanso.
   * *Módulo Pomodoro:* O usuário pode optar por iniciar o cronômetro 
   Pomodoro (25 minutos de foco ativo); ao finalizar o ciclo com sucesso, o 
   sistema reconhece a dedicação nos estudos e concede a recompensa 
   correspondente.
   * *Loja do Aplicativo:* O usuário acessa a loja para adquirir itens 
   cosméticos, enviando pedidos de compra que podem ser liquidados via 
   **Moeda XP** (adquirida no foco) ou via **Dinheiro Real** (compras 
   *in-app* - IAP).

2. **Pool: Sistema (App Pet Virtual):**
   * *Temporizadores e Alertas:* O sistema gerencia o horário de cuidado do 
   pet e dispara notificações push assíncronas para o usuário.
   * *Processamento de Recompensas e Compras:* Ao receber um pedido de 
   compra, verifica o tipo de transação via gateway exclusivo. Caso a compra 
   seja via XP, o saldo do usuário é validado: se suficiente, os pontos são 
   deduzidos, o item é liberado no inventário (banco de dados) e a 
   confirmação é entregue ao jovem; caso contrário, é emitida notificação de 
   cancelamento/saldo insuficiente.
   * *Integração com Gateway de Pagamento:* Caso a compra envolva dinheiro 
   real, o sistema suspende a operação até solicitar a aprovação na raia 
   parental. Mediante autorização confirmada, processa a cobrança via App 
   Store / Google Play Store e entrega o item cosmético.

3. **Pool: Controle Parental (Responsáveis):**
   * *Segurança e Autorização:* Recebe a solicitação da compra IAP contendo 
   o valor e a descrição do produto.
   * *Validação por Senha/PIN:* O responsável revisa a transação e insere a 
   credencial de segurança familiar (PIN Parental).
   * *Decisão:* Se aprovada, o envio de autorização é repassado ao sistema 
   para liquidação da compra; caso contrário, a rejeição encerra a cobrança 
   e notifica a recusa ao usuário sem efetivar débitos.

Comparando os dois modelos, fica evidente que cada gateway do fluxo proposto 
corresponde a uma refatoração direta de um fator crítico do sistema original: 
a ausência de mecanismo de pausa (Figura 4) deu lugar às notificações 
assíncronas; a evolução opaca por "care mistakes" deu lugar à Loja de XP 
transparente; e o isolamento do dispositivo, sem controle parental, deu 
lugar ao módulo de Controle Parental com autenticação via PIN.
...
|---|---|---|---|
|[Pabloo8](https://github.com/Pabloo8) | Inicio do Relatório, incluindo Foco 01, 02 e 03| [Commit 64667a4](https://github.com/UnBArqDsw2026-2-Turma02/2026.2-T02-_G4_ProjetoJogo_Entrega_01/commit/64667a426ccd8e2e10c48c4981a2b4ec87e2ba34) | 27/08/2026 |
|[Pabloo8](https://github.com/Pabloo8) | limpeza e correção do template| [Commit 68d26e3](https://github.com/UnBArqDsw2026-2-Turma02/2026.2-T02-_G4_ProjetoJogo_Entrega_01/commit/68d26e37aa05d4fe353503c93f48358c6518566a) | 27/08/2026 |
|[Pabloo8](https://github.com/Pabloo8) | adição de contribuiçoes dos integrantes| [Commit 7cd31f6](https://github.com/UnBArqDsw2026-2-Turma02/2026.2-T02-_G4_ProjetoJogo_Entrega_01/commit/7cd31f62d4c295d6415cac7dec07127b5a5b3485) | 27/08/2026 |
|[Pabloo8](https://github.com/Pabloo8) | correção de Significancia de contribuição| [Commit 774b273](https://github.com/UnBArqDsw2026-2-Turma02/2026.2-T02-_G4_ProjetoJogo_Entrega_01/commit/774b273c40f192146f2b689b699d5dffde511531) | 26/08/2026 |
|[luabrantess](https://github.com/luabrantess) | criação do rich picture| [Evidência](../Evidências/1.1.2.SubEquipe_02.md#evidências-de-elaboração) | 27/08/2026 |
|[luabrantess](https://github.com/luabrantess) | criação do BPMN | [Evidência](../Evidências/1.1.2.SubEquipe_02.md#evidências-de-elaboração-2) | 27/08/2026 |
|[luabrantess](https://github.com/luabrantess) | criação da Engenharia Reversa | [Evidência](../Evidências/1.1.2.SubEquipe_02.md#evidências-de-elaboração-3) | 27/08/2026 |
